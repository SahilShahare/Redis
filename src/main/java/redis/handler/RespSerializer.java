...
public class RespSerializer {
    ...
    public String serializeStreamReadEntryList(List<Pair<String, List<StreamEntry>>> lst) {
        if (lst.isEmpty()) {
            return NULL_ARRAY;
        }
        StringBuilder res = new StringBuilder("*").append(lst.size()).append("\r\n");
        lst.forEach(part -> res.append(
                serializeStreamReadEntry(part.key(), part.value())));
        return res.toString();
    }
}
